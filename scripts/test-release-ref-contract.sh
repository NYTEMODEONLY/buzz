...
(
  cd "$tmp"
  GITHUB_REF=refs/tags/desktop-v1.2.3 GITHUB_SHA=$(git rev-parse HEAD) "$verify" desktop-v 1.2.3
)

if (
  cd "$tmp"
  GITHUB_REF=refs/heads/main GITHUB_SHA=$(git rev-parse HEAD) "$verify" desktop-v 1.2.3
); then
...
"$repo_root/scripts/test-nytemode-naming-contract.sh"
"$repo_root/scripts/test-nytemode-release-contract.sh"
"$repo_root/scripts/test-nytemode-local-updater-contract.sh"
"$repo_root/scripts/test-nytemode-upstream-sync-contract.sh"
