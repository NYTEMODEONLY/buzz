...
mod custom_harness_commands;
mod identity_safety;
mod post_install_verification;

pub use custom_harness_commands::{delete_custom_harness, save_custom_harness};
use identity_safety::{
    active_directory_events, append_missing_owner_managed_agents, latest_directory_events,
    latest_owner_managed_event_versions, owner_managed_agents, retired_owner_managed_pubkeys,
};
...
pub async fn list_relay_agents(state: State<'_, AppState>) -> Result<Vec<RelayAgentInfo>, String> {
    ...
    // Fetch the public agent directory and the current owner's durable managed
    // declarations independently. The second lane is the identity authority:
    // an isolated Desktop install may not hold an agent's local key, and a
    // capped directory result may omit an older canonical profile.
    ...
    // Supplement the broad directory query with exact canonical authors, then
    // retain only the newest profile per author.
    ...
    // A deleted kind:30177 declaration retires the old directory profile. Only
    // the current owner may tombstone that exact coordinate, and a later live
    ...
    let owner_managed_versions =
        latest_owner_managed_event_versions(&owner_managed_events, &owner_pubkey);
    let retired_owner_managed_pubkeys = retired_owner_managed_pubkeys(
        &retired_owner_managed_events,
        &owner_pubkey,
        &owner_managed_versions,
    );
    let directory_events =
        active_directory_events(directory_events, &retired_owner_managed_pubkeys);
    ...
    append_missing_owner_managed_agents(&mut agents, &owner_managed_agents);
    Ok(agents)
}
