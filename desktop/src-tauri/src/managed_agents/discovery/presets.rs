...
pub(super) const PRESET_HARNESSES: &[PresetHarness] = &[
    ...
    PresetHarness {
        id: "hermes",
        label: "Hermes Agent",
        // The standard Hermes installer exposes `hermes` on the login-shell
        // PATH. Its `hermes-acp` console script remains inside Hermes' venv,
        // so resolving that name fails on a default installation.
        command: "hermes",
        args: &["acp"],
